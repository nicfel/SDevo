...
import bdmmprime.mapping.ODESystem;
 ...
 */
public class FlatTypeMappedTree extends CalculationNode implements Loggable {
...
//    	System.out.println(Arrays.toString(startTypeProbs));
        ...
        double[][] rates = new double[param.getNTypes()][param.getNTypes()];
        double[][] ratesPrime = new double[param.getNTypes()][param.getNTypes()];
        
        double[] startTypeProbsPrime = new double[param.getNTypes()];
        
        double[] totalRate = new double[param.getNTypes()]; 
        double[] totalRatePrime = new double[param.getNTypes()]; 
        ...
        for (integrationStep=0; integrationStep< FORWARD_INTEGRATION_STEPS; integrationStep++) {
            ...
            for (int currentType = 0 ; currentType < param.getNTypes(); currentType++) {
            	startTypeProbsPrime[currentType] = startTypeProbs[currentType];
            	totalRatePrime[currentType] = getTotalFowardsRate(currentType, tprime, subtreeRoot, ratesPrime[currentType]);               	
            }
            ...
            for (int a = 0; a < param.getNTypes(); a++) {
            	nodeTime[subtreeRoot.getNr()*param.getNTypes()+a] += (startTypeProbs[a]+startTypeProbsPrime[a])/2*dt;
            }
            ...
            double[] tmp2 = totalRate;
            totalRate = totalRatePrime;
            totalRatePrime = tmp2;
            
            double[] tmp3 = startTypeProbs;
            startTypeProbs = startTypeProbsPrime;
            startTypeProbsPrime = tmp3;
        ...
        switch(getNodeKind(subtreeRoot)) {
            ...
            case SA:
                forwardSimulateSubtree(subtreeRoot.getNonDirectAncestorChild(), endTime, startTypeProbs);
                break;
...
//                System.out.println(Arrays.toString(childTypes[1]));
    ...
    private double[][] sampleChildTypes(Node node, double[] parentTypeProbs) {
        ...
        double[][] jointprobs = new double[param.getNTypes()][param.getNTypes()];
        ...
        for (int parentType=0; parentType<param.getNTypes();parentType++) {
	        for (int type1=0; type1<param.getNTypes(); type1++) {
	            for (int type2=0; type2<param.getNTypes(); type2++) {
	                ...
	                if (type1 != parentType && type2 != parentType) {
//	                    probs[parentType][type1][type2] = 0.0;
	                    continue;
	                }
	
	                if (type1 == type2) {
	                	jointprobs[type1][type1] += parentTypeProbs[parentType]*param.getBirthRates()[interval][type1]
	                            *y1[param.getNTypes()+type1]*y2[param.getNTypes()+type1];
	                } else {
	                	jointprobs[type1][type2] += parentTypeProbs[parentType]*param.getCrossBirthRates()[interval][type1][type2]
	                            * 0.5 * (y1[param.getNTypes()+type1]*y2[param.getNTypes()+type2]
	                            + y1[param.getNTypes()+type2]*y2[param.getNTypes()+type1]);
	                }
...
//        System.exit(0);
        ...
        for (int type1=0; type1<param.getNTypes(); type1++) {
            for (int type2=0; type2<param.getNTypes(); type2++) {
            	probs[0][type1] += jointprobs[type1][type2];
            	probs[1][type2] += jointprobs[type1][type2];
            }
        }
//        for (int type1=0; type1<param.getNTypes(); type1++) 
//        	System.out.println(Arrays.toString(jointprobs[type1]));
//
//        for (int type1=0; type1<param.getNTypes(); type1++) 
//        	System.out.println(Arrays.toString(probs[type1]));
...
//        throw new IllegalStateException("Internal node child type sampling loop fell through.");
	...
	public double getNodeTime(int nr, int i) {
		return nodeTime[nr*param.getNTypes()+i];
	}
	
	public double getNodeTime(Node n, int i) {
		int index = -100;
		if (n.isLeaf()) {
			for (Node nodes : untypedTree.getExternalNodes()) {
				if (nodes.getID().contentEquals(n.getID())) {
					index = nodes.getNr();
					break;
				}
			}				
...
//		System.out.println(typedTree.getID());
//		typedTree.remapForLog(sample);


		out.print(toNewick(untypedTree.getRoot(), null, null));
		out.print(";");
    }
	...
	String toNewick(Node node, List<Function> metadataList, BranchRateModel.Base branchRateModel) {
		...
		if (!node.isLeaf()) {
			if (!node.isRoot()) {
				...
				double r1 = getNodeTime(node.getNr(),0)/(node.getParent().getHeight()-node.getHeight());
				double r2 = getNodeTime(node.getNr(),1)/(node.getParent().getHeight()-node.getHeight());
				
//				System.out.println(typedTreeInput.get().getNode(node.getNr()));
...
//				buf.append(r1 + "," + r2 + "}]");
				...
				double diff = getNodeTime(node.getNr(),0)+getNodeTime(node.getNr(),1) - (node.getParent().getHeight()-node.getHeight());
				
				if (Math.abs(diff)>10E-14 || r1>1 || r2>1) {
					System.out.println(diff + " " +r1 + " " + r2 + " too high");
				}
			}
...
//			buf.append(getNodeTime(node.getNr(),0) + "," +getNodeTime(node.getNr(),1) + "}");
//			buf.append( + "="
